-- ======================================================
-- SWILL MENU + ПОРТАЛЫ (С ПЛАВАЮЩЕЙ КНОПКОЙ)
-- ======================================================
...
floatingBtn.Position = UDim2.new(0.8, 0, 0.15, 0)
...
floatLabel.TextSize = 30
floatLabel.Font = Enum.Font.GothamBold
floatLabel.Parent = floatingBtn

-- Свечение вокруг кнопки
local glowFrame = Instance.new("Frame")
glowFrame.Size = UDim2.new(1.5, 0, 1.5, 0)
glowFrame.Position = UDim2.new(-0.25, 0, -0.25, 0)
...
-- Анимация плавания кнопки
local floatTween = TweenService:Create(floatingBtn, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
    Position = UDim2.new(0.8, 0, 0.15, 10)
})
floatTween:Play()
...
floatingBtn.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        isDragging = false
        -- Возвращаем анимацию, но с новой позиции
        floatTween = TweenService:Create(floatingBtn, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
            Position = UDim2.new(0, floatingBtn.Position.X.Offset, 0, floatingBtn.Position.Y.Offset + 5)
        })
        floatTween:Play()
    end
end)
...
floatingBtn.TouchTap:Connect(toggleMenu)
...
print("[SWILL] Меню загружено. Нажми на летающую кнопку для открытия.")
